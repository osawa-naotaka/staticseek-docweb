---
title: Benchmark on Intel N100
---

A second benchmark was conducted using an **Intel N100** CPU to evaluate performance on lower-power devices.

### Benchmark Conditions
- **Hardware**: Intel N100 CPU
- **Environment**: Benchmarks were conducted in a controlled environment to ensure consistency.
- **Index Size**: The size of the index was monitored to evaluate the impact on performance.

#### **Exact Search Time (ms) (English)**
| Text Size | Linear | GPU | Inverted |
|-----------|--------|-----|----------|
| 538       | 1.10   | 1.21| 0.61     |
| 829       | 1.36   | 1.37| 0.37     |
| 1712      | 2.74   | 2.79| 0.43     |
| 3001      | 4.52   | 4.67| 0.74     |
| 3748      | 5.65   | 5.63| 0.53     |

---

#### **Fuzzy Search Time (ms) (English)**
| Text Size | Linear | GPU | Inverted |
|-----------|--------|-----|----------|
| 538       | 22.41  | 8.40| 0.65     |
| 829       | 35.17  | 10.42| 0.84    |
| 1712      | 72.04  | 15.49| 0.84    |
| 3001      | 122.94 | 22.52| 1.33    |
| 3748      | 152.29 | 26.61| 1.37    |

---

#### **Fuzzy Search Time (ms) (Japanese)**
| Text Size | Linear | GPU | Inverted |
|-----------|--------|-----|----------|
| 476       | 6.85   | 7.17| 0.37     |
| 789       | 11.58  | 6.90| 0.56     |
| 1305      | 18.43  | 7.27| 0.53     |
| 2394      | 34.15  | 8.51| 0.63     |
| 3020      | 42.80  | 9.30| 0.59     |

---

#### **Indexing Time (ms) (English)**
| Text Size | Linear | GPU | Inverted |
|-----------|--------|-----|----------|
| 538       | 110.82 | 132.24| 4896   |
| 829       | 218.48 | 206.16| 7536   |
| 1712      | 383.70 | 386.68| 16064  |
| 3001      | 661.34 | 639.38| 28548  |
| 3748      | 790.48 | 788.48| 36279  |

---

#### **Index Size (Gzipped, kbyte) (English)**
| Text Size | Linear | GPU | Inverted |
|-----------|--------|-----|----------|
| 538       | 191    | 191 | 91       |
| 829       | 294    | 294 | 131      |
| 1712      | 607    | 607 | 247      |
| 3001      | 1057   | 1057| 411      |
| 3748      | 1324   | 1324| 505      |

---

#### **Index Size (Gzipped, kbyte) (Japanese)**
| Text Size | Linear | GPU | Inverted |
|-----------|--------|-----|----------|
| 476       | 165    | 165 | 136      |
| 789       | 272    | 272 | 229      |
| 1305      | 452    | 452 | 366      |
| 2394      | 837    | 837 | 644      |
| 3020      | 1053   | 1053| 813      |

### Conclusion

In summary, the benchmarks conducted on the Intel N100 CPU demonstrate the performance characteristics of different indexing strategies. While the `LinearIndex` provides reliable performance, the `GPU` and `Inverted` indexes offer significant speed improvements, especially for larger datasets. When selecting an index type, consider the specific requirements of your application and the hardware capabilities available.
