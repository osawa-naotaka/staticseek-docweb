...
An inverted index is an array of "terms and lists of document IDs containing those terms." The list of document IDs is called a "posting list." The inverted index is created in three steps:

1. Assign an ID to each document, segment the document, and create a list of terms
2. For each term in the document, create a tuple (term, posting list) and add this tuple to the inverted index. If the term is already registered, add the document ID to the posting list
